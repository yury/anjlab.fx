...
namespace AnjLab.FX.UI
{
    public class RuNumberFormat
    {
        ...
        public static string Case(int val, string oneTwoFivePattern)
        {
            string[] words = oneTwoFivePattern.Split(',');
            if (words.Length != 3)
                return oneTwoFivePattern;

            return Case(val, words[0], words[1], words[2]);
        }
    };
    ...
    public class RusCurrency
    {
            ...
            string juniorOne, string juniorTwo, string juniorFive)
        {
            ...
            if (0 == n) r.Append("0 ");
            if (n % 1000 != 0)
                r.Append(RuNumberFormat.Str(n, male, seniorOne, seniorTwo, seniorFive));
            else
                r.Append(seniorFive);

            n /= 1000;

            r.Insert(0, RuNumberFormat.Str(n, false, "тысяча", "тысячи", "тысяч"));
            n /= 1000;

            r.Insert(0, RuNumberFormat.Str(n, true, "миллион", "миллиона", "миллионов"));
            n /= 1000;

            r.Insert(0, RuNumberFormat.Str(n, true, "миллиард", "миллиарда", "миллиардов"));
            n /= 1000;

            r.Insert(0, RuNumberFormat.Str(n, true, "триллион", "триллиона", "триллионов"));
            n /= 1000;

            r.Insert(0, RuNumberFormat.Str(n, true, "триллиард", "триллиарда", "триллиардов"));
            if (minus) r.Insert(0, "минус ");

            r.Append(remainder.ToString("00 "));
            r.Append(RuNumberFormat.Case(remainder, juniorOne, juniorTwo, juniorFive));
